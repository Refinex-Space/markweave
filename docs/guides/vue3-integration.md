...
Vue 3 exposes the same `on-search-controller-change` lifecycle as React/Vue 2. Large queries may report `execution.status="searching"`; counts and replacement become authoritative at `ready`. Use the exported `prepareMarkweaveEditorForOutput` before browser print or DOM/PDF capture.
