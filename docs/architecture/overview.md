...
- link editing: the floating toolbar opens an inline link popover for selected text, with apply, open, and remove actions; in editable Live mode the shared link plugin projects normalized `[label](target "title")` source only around the active inline link, keeps the projection outside document storage/history, commits safe target edits on Enter or blur, discards drafts on Escape, and leaves Ctrl/Cmd-click navigation to the existing safe-link path
