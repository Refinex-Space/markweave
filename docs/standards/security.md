...
- Video insertion can create an empty browser-side upload placeholder before a `src` exists. Local video files still require the host upload handler. Direct video URLs render as `<video>` without autoplay. YouTube and Bilibili sharing URLs are converted to iframe embeds, while whitelisted platform embed sources keep their original query strings except that autoplay is forced off. Do not resolve network redirects or accept arbitrary iframe hosts.
