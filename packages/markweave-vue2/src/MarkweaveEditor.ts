...
type VueBlockOption = readonly [id: FloatingToolbarTurnIntoId, label: string, icon: Component];
type MarkweaveEmojiItem = MarkweaveMessages["slash"]["emojiItems"][number];
type MarkweaveTocItem = MarkweaveTocState["items"][number];

const toolbarTurnIntoIconMap: Record<FloatingToolbarTurnIntoId, Component> = {
  paragraph: TypeIcon,
  "heading-1": Heading1,
  "heading-2": Heading2,
  "heading-3": Heading3,
...
function currentBlockType(editor: CoreEditor, messages: MarkweaveMessages) {
  return getCurrentFloatingToolbarBlockType(editor, messages).label;
}
const VueFloatingToolbar = defineComponent({
  ...
  setup(props) {
    ...
    const blockOptions = computed<readonly VueBlockOption[]>(() =>
      getFloatingToolbarTurnIntoOptions(props.messages).map((option) => [option.id, option.label, toolbarTurnIntoIconMap[option.id]] as const),
    );
    const textColorOptions = computed<readonly VueColorOption[]>(() =>
      getFloatingToolbarTextColorOptions(props.messages).map((option) => [option.id, option.value, option.label] as const),
    );
    const highlightOptions = computed<readonly VueColorOption[]>(() =>
      getFloatingToolbarHighlightColorOptions(props.messages).map((option) => [option.id, option.value, option.label] as const),
    );
    const moreActions = computed<readonly FloatingToolbarMoreActionId[]>(() => getFloatingToolbarMoreActions(props.messages).map((action) => action.id));
    const applyLink = () => {
      if (!safeHref.value) {
        return;
      }
    ...
    const submitAskAi = async (promptOverride?: string) => {
      if (!askAiEnabled.value || props.askAi?.enabled !== true || !askAiSelection.value) return;
      ...
      try {
        ...
        if (askAiRequestId.value !== request.id || controller.signal.aborted) return;
        askAiStreamScheduler.cancel();
        ...
        if (askAiRequestId.value !== request.id) return;
        askAiStreamScheduler.cancel();
    ...
    const handleAskAiEscape = (event: KeyboardEvent) => {
      if (event.key !== "Escape" || openMenu.value !== "ask-ai") return;
      event.preventDefault();
    ...
    watch(() => [openMenu.value, askAiPhase.value, askAiMarkdown.value, askAiError.value] as const, ([menu]) => {
      if (!menu) {
        return;
      }
          ...
          ? h("div", { class: "markweave-ask-ai-composer", "data-variant": askAiPhase.value === "result" ? "follow-up" : "prompt" }, [
              h("textarea", {
                ...
                onKeydown: (event: KeyboardEvent) => {
                  if (event.key === "Enter" && !event.shiftKey && !event.isComposing) { event.preventDefault(); void submitAskAi(); }
                },
              }),
              h("button", {
                type: "button",
          ...
          ...blockOptions.value.map(([id, label, icon]) =>
            h(
              "button",
              {
                ...
                onMousedown: preventVuePointerFocusLoss,
                onClick: () => {
                  setFloatingToolbarTurnInto(props.editor, id);
                  openMenu.value = null;
                },
              },
          ...
          h("span", { key: id, class: "markweave-floating-toolbar-more-item" }, [
            h(
              "button",
              {
                ...
                onClick: () => {
                  runFloatingToolbarMoreAction(props.editor, id);
                  openMenu.value = null;
                },
              },
...
const VueTableControls = defineComponent({
  ...
  setup(props) {
    ...
    const hasCellMenuCommands = computed(() => {
      void cellMenuCommands.value;
      return focusState.value.active && getTableMenuItems(props.editor, "selection", { askAiEnabled: askAiEnabled.value }).length > 0;
    });
    ...
    const menu = () => {
      ...
      const menuItemChildren = menuItems.value.map((item, index) => {
        ...
        return h("button", {
          ...
          onMouseenter: () => { openSubmenu.value = item.submenuId; },
      ...
      if (openSubmenu.value === "color" && (openMenu.value === "row" || openMenu.value === "column")) {
        menuChildren.push(h("div", { ref: submenuRef, class: "markweave-table-submenu markweave-table-color-menu", role: "menu", "aria-label": props.messages.table.submenus.color, "data-testid": "markweave-table-color-menu", "data-positioned": submenuPosition.value ? "true" : "false", "data-placement": submenuPosition.value?.placement, style: submenuPosition.value ? { left: `${submenuPosition.value.left}px`, top: `${submenuPosition.value.top}px`, maxHeight: `${submenuPosition.value.maxHeight}px` } : undefined, onKeydown: onMenuKeydown }, [
          ...
          }, [h("span", { class: "markweave-table-color-swatch markweave-table-text-color-swatch", "aria-hidden": "true", style: { backgroundColor: option.textColor ?? "var(--markweave-text)" } }), h("span", null, props.messages.table.colors[option.id].text)])),
          ...
          }, [h("span", { class: "markweave-table-color-swatch", "aria-hidden": "true", style: { backgroundColor: option.backgroundColor } }), h("span", null, props.messages.table.colors[option.id].background)])),
        ]));
      }
...
export function useMarkweaveEditorController(options: MarkweaveVue2EditorControllerOptions = {}): MarkweaveVue2EditorController {
  ...
  const commandControllerEditor = editorRef.value;
  setMarkweaveCommandRegistry(commandControllerEditor, commandRegistry.value, (error) => {
    slashCommandError.value = error.message;
    options.onCommandError?.(error);
  });
  setMarkweaveCommandContentFormat(commandControllerEditor, activeFormat);
  watch(commandRegistry, (registry) => {
    if (!editorRef.value) return;
    setMarkweaveCommandRegistry(editorRef.value, registry, (error) => {
      slashCommandError.value = error.message;
      options.onCommandError?.(error);
    });
  });
  ...
  const actions: MarkweaveVue2EditorControllerActions = {
    ...
    setContent: (nextContent, setContentOptions = {}) => {
      ...
      editor.value.commands.setContent(nextContent, { contentType: getMarkweaveContentType(nextFormat), emitUpdate: setContentOptions.emitUpdate ?? false });
